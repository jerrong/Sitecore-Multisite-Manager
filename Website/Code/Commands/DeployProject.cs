...
{
    public class DeployProject : Sitecore.Shell.Framework.Commands.Command
    {
        ...
        public override void Execute(CommandContext context)
        {
            ...
            var items = context.Items[0];
             ...
             ProgressBox.Execute("Deploying Site", "Running Build Steps", StartDeployment, new object[] { context.Items[0].ID.ToString() });

        }
        public void StartDeployment(params object[] parameters)
        {
            using (new Sitecore.SecurityModel.SecurityDisabler())
            {
                Sitecore.Data.Database db = Factory.GetDatabase("master");
                var deploymentSteps = db.Items.Database.GetItem("/sitecore/system/Site Manager/Deployment Pipeline");

                foreach (Item step in deploymentSteps.Children)
                {
                    Sitecore.Context.Job.Status.Messages.Add(step.Name);
                    var items = db.GetItem(parameters[0].ToString());
                    foreach (Item project in items.Children)
                    {
                        RunPowershellScript(@"C:\Export\" + project.Name + "\\SiteName\\SiteEnvironment", "go.cmd",
                                            step.Fields["Task Name"].Value);
                    }
                    Thread.Sleep(5000);
                }
